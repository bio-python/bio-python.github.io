...
1. Extract *protein product* and *description* in the following tab-separated(tab) [file](./data/Thamnophis_Protein_table.txt) into a comma-separated (csv) file.
...
3. Search the following 8 nucleotide sequence in this [file](./data/dna.example.fasta): "tagctgatc". Report the lines in which they occur.
...
1. Given two numbers a and b. Find the sum of all odd numbers between the two. --Use: Logical Thinking.
2. Given a file, print only the even numbered lines from the [file](./data/Even-lines-file.txt).  Assume 1-based numbering of lines. -- Application: Picking only the headers/sequence from a DNA file.
3. Given a [string](./data/word-count.txt), count the number of times each word appears. -- Application: k-mer counting from a DNA file.(i.e) finding regulatory regions for genes.
